...
#db = client.chatdb
client = pymongo.MongoClient("mongodb://127.0.0.1:27017/chatdb")
db = client["chatdb"]
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    token = jwt.encode(data, JWT_SECRET, algorithms=['HS256'])
...
async def register_user(model: User):
    ...
    if exists:
        ...
        obj_id = db.users.insert_one({"username": model.username, "email": model.email, "name": model.full_name,
